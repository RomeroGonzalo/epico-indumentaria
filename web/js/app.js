...
// Lógica: selector de cliente, catálogo, carrito, WhatsApp
// ============================================================
...
function updatePriceUI() {
  ...
  if (ct.discount === 0) {
    ...
    const pct = ct.discount * 100;
    pillText.innerHTML = `Precios: <strong>${ct.label} (−${pct}%)</strong>`;
    priceModePill.querySelector('.pill-icon').textContent = ct.icon;
  }

  // Actualizar precios en todas las tarjetas
...
function calcPrice(basePrice) {
  const ct = CLIENT_TYPES[currentClientType];
  return Math.round(basePrice * (1 - ct.discount));
}
...
function buildProductCard(p) {
  ...
  // Color swatch selection (with optional image swap per color)
  card.querySelectorAll('.color-swatch').forEach(sw => {
    sw.addEventListener('click', () => {
      ...
      if (sw.dataset.image) {
        ...
        imgWrap.innerHTML = `<img class="product-img" src="${sw.dataset.image}" alt="${sw.dataset.color}">`;
      }
    });
  });
  ...
  card.querySelector('.btn-add-cart').addEventListener('click', () => {
    ...
    const unitPrice = calcPrice(p.price);
    addToCart(p, color, size, qty, unitPrice);
  });
...
function addToCart(product, color, size, qty, unitPrice) {
  ...
  if (existing) {
    existing.qty += qty;
    existing.unitPrice = unitPrice; // refresh price if client type changed
  } else {
    cart.push({ key, sku: product.sku, name: product.name, emoji: product.emoji, color, size, qty, unitPrice, basePrice: product.price });
  }
...
function renderCart() {
  // Recalc prices based on current client type
